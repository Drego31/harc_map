 ...
 * Can read multiple objects
 * @param collectionName {string} - name of collection we are operating
 * @param arrayOfQuery {array} - array of mongodb documents to delete
 * @return {Promise}
 * @public
 */
function remove(collectionName, arrayOfQuery) {
	return new Promise((resolve, reject) => {
		__getCollection(collectionName, (errorColl, collection) => {
			if (errorColl) {
				reject(errorColl);
			} else {
				collection.deleteMany(arrayOfQuery, (errorQuery, result) => {
					if (errorQuery) {
						reject(errorQuery);
					} else {
						resolve(result);
