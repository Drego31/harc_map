...
function __connect (callback) {
  ...
  if (connection) {
    console.log('use_existing', (Date.now() - timestamp) / 1000 + ' s');
    callback(null, connection);
  } else {
    MongoClient.connect(connectionString, { useUnifiedTopology: true }).then(clientConnection => {
      console.log('new_connection', 'success', (Date.now() - timestamp) / 1000 + ' s');
      connection = clientConnection;
      callback(null, connection);
    }).catch(error => {
      console.log('new_connection', 'error', (Date.now() - timestamp) / 1000 + ' s', error);
