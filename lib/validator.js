...
const ValidateCodes = Object.freeze({
  ...
  POINT_IS_ACTIVE_INCORRECT: 1016,
  EVENT_ID_NOT_EXIST: 1017,
  // 20## - database errors,
  DATABASE_DATA_ERROR: 2000,
  DATABASE_NO_RESULT_ERROR: 2001,
  DATABASE_CONNECTION_ERROR: 2002,
...
class Validator {
  validatePoint (point) {
    ...
    if (tools.isNullOrEmpty(point.pointValue)) {
      throw ValidateCodes.POINT_VALUE_IS_EMPTY;
    }
    if (tools.isNullOrEmpty(point.pointShape)) {
      throw ValidateCodes.POINT_VALUE_IS_EMPTY;
    }
    if (tools.isNotBoolean(point.pointIsActive)) {
      throw ValidateCodes.POINT_IS_ACTIVE_INCORRECT;
    }
  }
