...
function responseDatabaseError (response, responseObject, error) {
  responseObject.error = validateCodes.DATABASE_DATA_ERROR;
  responseError(response, 500, error, responseObject);
}

function responseDatabaseNoData (response, responseObject) {
  responseObject.error = validateCodes.DATABASE_NO_RESULT_ERROR;
  responseError(response, 500, null, responseObject);
}
